...
if len(sys.argv) < 2:
	...
	subprocess.call("touch remind.swap", shell=True)
	with open('remind','r+') as f:
		with open('remind.swap','r+') as fswap:
			for line in f:
				data =line.split(" ",1)
				if int(data[0]) - 1 == 0:
		...
		fswap.closed
	f.closed
	subprocess.call("rm remind", shell=True)
	subprocess.call("mv remind.swap remind", shell=True)
elif len(sys.argv) < 3:
	print("Missing argument\nremindme [time] [instruction]")
else:
	with open('remind','r+') as f:
